/*
	...
	Date: March-17-2015
	Specific Scenariz Cron management for Sarah
...
nedbClient.prototype.dbinit = function () {
	var dbstore = require('../nedb'),
	    path = __dirname + '/Scenariz.db',
	    Scenarizdb = new dbstore({ filename: path});
	Scenarizdb.loadDatabase();
	return Scenarizdb;   
}
...
nedbClient.prototype.dbcron = function (callback) {
	...
	client.Scenarizdb.find({ Exec : "true" }, function (err, docs) {
		if (err){
